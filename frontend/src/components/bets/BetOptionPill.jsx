    ...
    <Button
      ...
      className={`w-full min-w-0 rounded-2xl border text-left transition-all duration-200 ${
        ...
        selected
          ...
          : "border-base-300 bg-base-200 hover:border-primary/30 hover:bg-base-300"
      } ${disabled ? "cursor-not-allowed opacity-60" : "cursor-pointer"}`}
    >
      <div className="flex min-w-0 items-center justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p className="wrap-break-word text-sm font-semibold sm:text-md">{label}</p>
          <p className="text-xs sm:text-sm text-base-content/60">{t("pages.bets.detail.odds")}</p>
        </div>
