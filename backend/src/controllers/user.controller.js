...
const transactionColumnKeys = Object.freeze(
  Object.keys(TRANSACTION_SELECTABLE_COLUMNS),
)
...
const usersListQuerySchema = createListQuerySchema({
  ...
  minBalance: z
    .preprocess((value) => toOptionalNumber(value), z.number().finite())
    .optional(),
  maxBalance: z
    .preprocess((value) => toOptionalNumber(value), z.number().finite())
    .optional(),
  ...
  filters: createStructuredFiltersSchema(
    USER_FILTER_FIELDS,
    userFilterValueSchemas,
  ),
})
...
const parseListQuery = (query, schema, fieldValueSchemas = {}) => {
  ...
  const singleFilterField =
    parsedQuery.data.filterField ??
    parsedQuery.data.filterBy ??
    parsedQuery.data.column
  if (singleFilterField) {
    const singleFilterValue =
      parsedQuery.data.filterValues ??
      parsedQuery.data.filterValue ??
      parsedQuery.data.value
    ...
    if (!parseSingleFilter?.success) {
      return {
        error: parseSingleFilter?.error?.issues ?? [
          { message: "INVALID_FILTER_VALUE" },
        ],
      }
    }
  }
...
const createUserByAdmin = async (req, res) => {
  try {
    if (!req.body)
      return res.status(400).json({ code: "AUTH_NO_DATA_PROVIDED" })
    ...
    const { username, email, password, role, balance } = parsed.data
...
const getTransactions = async (req, res) => {
  try {
    ...
    const txs = await User.findTransactionsByUser(
      req.user.id,
      page,
      limit,
      filters,
